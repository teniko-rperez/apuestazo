...
export function Navbar() {
  ...
  return (
    <>
      {/* ═══ MOBILE: Top header ═══ */}
      <header className="lg:hidden fixed top-0 inset-x-0 z-50 glass bg-[#0f172a]/95 border-b border-white/5">
        <div className="h-14 px-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleRefresh} disabled={refreshing}
              className={cn(
                "h-8 px-4 text-[11px] font-bold rounded-lg transition-all active:scale-95",
                refreshing
      ...
      {menuOpen && (
        <div className="lg:hidden fixed inset-0 z-[60]" onClick={() => setMenuOpen(false)}>
          ...
          {/* Menu panel */}
          <div
            className="absolute bottom-0 inset-x-0 bg-white rounded-t-3xl shadow-2xl pb-safe animate-slide-up"
              ...
              {MORE_NAV.map((n) => {
                ...
                return (
                  <Link key={n.href} href={n.href} onClick={() => setMenuOpen(false)}
                    className={cn(
                      "flex items-center gap-4 h-12 px-4 rounded-xl transition-all active:scale-[0.98]",
                      active
                        ? "bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-lg shadow-orange-500/20"
                        : "text-gray-600 hover:bg-gray-50"
                    )}>
                    <span className={cn(active ? "text-white" : "text-gray-400")}>{n.icon}</span>
          ...
          {NAV_ITEMS.map((n) => {
            ...
            return (
              <Link key={n.href} href={n.href}
                ...
                <span className={cn("transition-transform", active && "scale-110")}>{n.icon}</span>
                <span className="text-[9px] font-bold leading-none">{n.mobileLabel}</span>
              </Link>
            );
          })}
